    ...
    <div className="bg-gray-800">
          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-baseline space-x-4"></div>
              {navLinks.map((link, index) => (
                <a className="text-gray-300 transition-all duration-500 hover:bg-gray-600 hover:text-white px-3 py-2 rounded-md font-medium" key={index} href={link.link}>{link.title}</a>
              ))}
      ...
      {open ? ( 
        <div className="md:hidden">
          ...
          {navLinks.map((link, index) => (
            <a className="text-gray-300 hover:bg-gray-700 hover:text-white block px-3 py-2 rounded-md text-base font-medium" key={index} href={link.link}>{link.title}</a>
          ))}
          </div>
        </div>
      ): null}
